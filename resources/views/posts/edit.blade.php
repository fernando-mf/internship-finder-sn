...
@section('content')
    ...
    <div class="row">
        <div class="col-md-10 col-md-offset-1">
            <form class="form-horizontal" method="POST" action="{{ route('posts.update', ['id' => $post->id]) }}">
                ...
                <div class="panel panel-default">
                    ...
                    <div class="panel-body">
                        ...
                        @component('components.control', ['c_name' => 'body', 'c_description' => 'Description'])
                            <textarea rows="5" id="body" class="form-control" name="body" placeholder="Décrivez votre expérience de stage p.ex: Les tâches que vous avez faites, l'environnement de travail, la formation reçue dans la compagnie, etc">{{ old('body', $post->body) }}</textarea>
                        @endcomponent
                        ...
                        @component('components.control', ['c_name' => 'program', 'c_description' => 'Programme'])
                            <select name="program" id="program" class="form-control">
                                @foreach(App\Program::all()->where('code', '!=', '0') as $program)
                                    @if(old('program', $post->program_id) == $program->id)
                ...
                <div class="panel panel-default">
                    ...
                    <div class="panel-body">
                        @component('components.control', ['c_name' => 'co_name', 'c_description' => 'Nom'])
                            <input id="co_name" type="text" class="form-control" name="co_name" value="{{ old('co_name', $contact->name) }}">
                        @endcomponent

                        @component('components.control', ['c_name' => 'co_email', 'c_description' => 'Adresse courriel'])
                            <input id="co_email" type="email" class="form-control" name="co_email" value="{{ old('co_email', $contact->email) }}">
                        @endcomponent

                        @component('components.control', ['c_name' => 'co_phone', 'c_description' => 'Téléphone'])
                            <input id="co_phone" type="text" class="form-control" name="co_phone" value="{{ old('co_phone', $contact->phone) }}" placeholder="(123) 456-7890" onkeydown="backspacerDOWN(this,event)" onkeyup="backspacerUP(this,event)">
                        @endcomponent

                        @component('components.control', ['c_name' => 'co_ext', 'c_description' => 'Poste / Ext'])
                            <input id="co_ext" type="text" class="form-control" name="co_ext" value="{{ old('co_ext', $contact->ext_poste) }}">
                        @endcomponent
                    </div>
                </div>
